...
(defn eval-expo-with-examples [exp1 env1 selves1 val1
                               ...
                               exp3 env3 selves3 val3]
  (all
   (trace-lvars "eval" [exp1 exp2 exp3])
   (conde
     [(fresh [a*1 a*2 a*3]
             (conso 'list a*1 exp1)
             (conso 'list a*2 exp2)
     ...
     [(primitivo exp1) (symbolo exp2) (symbolo exp3)
      (== exp1 val1)
      (lookupo exp2 env2 val2)
      (lookupo exp3 env3 val3)]
     [(primitivo exp1) (primitivo exp2) (symbolo exp3)
      (== exp1 val1) (== exp2 val2)
      (lookupo exp3 env3 val3)]
     ...
     [(fresh [selfarg2 argv2 prevargv2 x2 body2 env-2 env+2 t2
             ...
             (== `(~'self ~selfarg2) exp2)
             (== `(~'self ~selfarg3) exp3)
             (not-in-envo 'self env2)
             (not-in-envo 'self env3)
             ...
             (eval-expo-with-examples (prim '()) env1 selves1 (prim '())
     ...
     [(fresh [a2 n2 a3 n3]
             ...
             (inco n3 val3))]
     [(fresh [a3 n3]
             (primitivo exp1) (== exp1 val1)
             (== `(~'zero) exp2) (not-in-envo 'zero env2) (zeroo val2)
             (== `(~'inc ~a3) exp3)
             (not-in-envo 'inc env3)
             (eval-expo-with-examples val1 env1 selves1 val1
                                      (prim '()) env2 selves2 (prim '())
                                      a3 env3 selves3 n3)
             (inco n3 val3))]
     [(primitivo exp1) (== val1 exp1)
      (primitivo exp2) (== val2 exp2)
